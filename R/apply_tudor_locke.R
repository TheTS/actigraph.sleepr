...
apply_tudor_locke_ <- function(data,
  ...
  data %>%
    ...
    summarise(timestamp = first(timestamp),
              sleep = first(sleep),
              duration = n(),
              nonzero_epochs = sum(axis1 > 0),
              activity_counts = sum(axis1)) %>%
    ...
    mutate(out_bed_time = timestamp + duration(duration, "mins"),
           sleep_fragmentation_index = movement_index + fragmentation_index,
           wake_after_onset = duration - total_sleep_time,
           ave_awakening = if_else(nb_awakenings > 0,
                                   wake_after_onset / nb_awakenings, 0),
           efficiency = 100 * total_sleep_time / duration,
           onset = timestamp, latency = 0) %>%
    rename(in_bed_time = timestamp) %>%
    select(in_bed_time, out_bed_time, onset, latency,
           efficiency, duration, activity_counts,
           nonzero_epochs, total_sleep_time,
           wake_after_onset, nb_awakenings, ave_awakening,
           movement_index, fragmentation_index,
           sleep_fragmentation_index) %>%
    mutate_at(vars(latency, activity_counts, nonzero_epochs,
